...
describe('AuthForm', () => {
  ...
  it('disables the email, GitHub, and Google buttons while GitHub sign-in is in flight', async () => {
  ...
  it('disables the email, Google, and GitHub buttons while Google sign-in is in flight', async () => {
    mocks.signInWithOAuth.mockReturnValue(new Promise(() => {})) // never resolves
    render(<AuthForm mode="signin" />)
    fireEvent.click(screen.getByRole('button', { name: /continue with google/i }))
  ...
  it('tracks a signup_attempt when Continue with Google is clicked in signup mode', async () => {
    mocks.pathname = '/signup'
    render(<AuthForm mode="signup" />)
    fireEvent.click(screen.getByRole('button', { name: /continue with google/i }))

    expect(mocks.track).toHaveBeenCalledWith('signup_attempt', { method: 'google' })
  })

  it('does not track a signup_attempt for Google sign-in on the signin screen', async () => {
    mocks.pathname = '/signin'
    render(<AuthForm mode="signin" />)
    fireEvent.click(screen.getByRole('button', { name: /continue with google/i }))
