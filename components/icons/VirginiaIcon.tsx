...
// Virginia state outline, traced from the public-domain (CC0) "Blank US
// Map (states only)" on Wikimedia Commons -- this path is Virginia's
// piece of that combined map, cropped to its own bounding box (including
// the Eastern Shore, geographically separate from the mainland):
// https://commons.wikimedia.org/wiki/File:Blank_US_Map_(states_only).svg
export default function VirginiaIcon({ size = 96, className = '' }: Props) {
  return (
    <svg
      width={size}
      height={size * (82.4 / 143.6)}
      viewBox="694.7 241.7 143.6 82.4"
      fill="currentColor"
      aria-hidden="true"
      className={className}
    >
